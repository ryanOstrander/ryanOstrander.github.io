...
function plotCoefficients(cHist) {

    for (i = 0; i < 3; i++) {

      pt = new Point(coeffList[i].re,coeffList[i].im , 2);
      if (frameCount % 8 == 0) cHist[i].push([pt.pixelX, pt.pixelY]);
      noStroke();
      fill(colors[i]);
      pt.plotPoint('a' + (i));
...
function calcFromCoef(a) {
  ...
  //sqrt(4a0 + a1)/2
  calcPoint = math.pow(math.divide(math.subtract(math.multiply(a[2], a[2]), math.multiply(math.multiply(a[1], 2) , a[0])), 2), 1/2);
  ...
  if ((frameCount % 5) == 0 && beginPath && !animationComplete) {
...
function drawHistoryList(histList) {
    ...
    for (let j = 0; j < 3; j++) {
      ...
      if (size > 5) {
        ...
        for (let i = 0; i < size - 2; i++) {
          ...
          if (histList[j][i+1][0] == -1 && histList[j][i+1][1] == -1) {
            stroke(color('red'), 1);
            i += 2;
          }
          else if (histList[j][i+1][0] == -2 && histList[j][i+1][1] == -2) {
            stroke(color('green'), 2);
            i += 3;
          }
          else if (histList[j][i+1][0] == -3 && histList[j][i+1][1] == -3) {
            stroke(color('blue'), 3);
            i += 2;
          }
          else {
            line(histList[j][i][0], histList[j][i][1], histList[j][i+1][0], histList[j][i+1][1]);
          }
