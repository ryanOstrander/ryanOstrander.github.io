...
function plotCoefficients() {
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (i == 0) {
        a0Hist.push([pt.pixelX, pt.pixelY]);
      }

      if (i == 1) {
        a1Hist.push([pt.pixelX, pt.pixelY]);
      }
...
function calcFromCoef(a) {
  ...
  //-(7a + 4a1)/3
  calcPoint = math.multiply(math.divide(math.subtract((math.multiply(7, a[0])), math.multiply(4, a[1])) , 3), -1);
...
function calcFromCoef2(a) {
  var calcPoint;
  var calcPlot;

  //(7a0 - 4a1)/3
  calcPoint = math.divide(math.subtract((math.multiply(7, a[0])), math.multiply(4, a[1])) , 3);
  // calculate4th(calcPoint);
  ...
  //save history every 10 frames
  if ((frameCount % 3) == 0 && beginPath && !animationComplete) {
   calcHistory2.push([calcPlot.pixelX, calcPlot.pixelY]); 
  }
...
function drawHistory(history, colorName) {
...
function sketchAxis(){
  ...
  textSize(14);
  text(topLeftBoundX, 498 - 30, height/4 - 5);
  text("-" + topLeftBoundX, 5, height/4 - 5);
  text(topLeftBoundY, 498/2 + 5, 15);
  text("-" + topLeftBoundY, 498/2 + 5, height/2 - 5);
  ...
  textSize(14);
  text(topRightBoundX, width - 20, height/4 - 8);
  text(topRightBoundY, (498/2)*3 + 5, 15);
  text("-" + topRightBoundX, (498/2)*2 + 10, height/4 - 5);
  text("-" + topRightBoundY, (498/2)*3 + 5 , height/2 - 5);
  ...
  textSize(14);
  text(bottomLeftBoundX, 498 - 23, (height/4 - 5) * 3);
  text("-" + bottomLeftBoundX, 5, (height/4 - 5)*3);
  text(bottomLeftBoundY, 498/2 + 5, (height/2) + 20);
  text("-" + bottomLeftBoundY, 498/2 + 5, height - 5);
  ...
  // textSize(14);
  // text(bottomrightBoundX, width - 20, (height/4 - 4)*3);
  // text(bottomrightBoundY, (498/2)*3 + 5, height/2 + 20);
  // text("-" + bottomrightBoundX, 498 + 10, (height/4 - 5)*3);
  // text("-" + bottomrightBoundX, (498/2)*3 + 5 , height - 5);
